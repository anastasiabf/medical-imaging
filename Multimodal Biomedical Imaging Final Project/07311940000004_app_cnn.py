...
class ConvolutionalNetwork(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv1 = nn.Conv2d(1, 16, 3, 1) #1color channel, 16 filters (output channel from layer1), 3x3 kernel, stride=1
        self.conv2 = nn.Conv2d(16, 32, 3, 1) #16 in channels, 32 out channels
        self.conv3 = nn.Conv2d(32, 64, 3, 1) #32 in channels, 64 out channels
        self.fc1 = nn.Linear(1*1*64, 120) #for fully connected layer, dgn 120 neurons
    ...
    def forward(self, X):
        ...
        X = F.max_pool2d(X, 2, 2)
        X = X.view(-1, 1*1*64) #-1 agar kita bisa memvariasikan batch size
        X = F.relu(self.fc1(X))
        X = F.relu(self.fc2(X))
        X = self.fc3(X)
...
def preprocess_image(image):
    ...
    preprocessed_image = preprocessed_image.unsqueeze(0)  # Menambahkan dimensi batch
...
def main():
    ...
    st.title("Classification MNIST Dataset Using Convolutional Neural Network")
